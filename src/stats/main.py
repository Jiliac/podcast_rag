...
def calculate_total_audio_duration(episodes: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    return {
        ...
        "average_duration_minutes": round(total_minutes / episodes_with_audio, 2)
        if episodes_with_audio > 0
        else 0,
    }
...
def calculate_episode_coverage_stats(episodes: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if days_covered > 0:
        ...
        avg_episodes_per_month = (
            len(dated_episodes) / months_covered if months_covered > 0 else 0
        )
    else:
...
def calculate_content_analysis_stats(episodes: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "average_characters_per_episode": round(
            total_characters / len(transcriptions), 0
        ),
        "shortest_episode": {
...
def calculate_data_quality_stats(episodes: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    total_fields = (
        len(episodes) * 3
    )  # 3 fields per episode (transcription, date, audio_url)
    complete_fields = (
        episodes_with_transcriptions + episodes_with_dates + episodes_with_audio_urls
    )
...
def print_stats(stats: Dict[str, Any]) -> None:
    ...
    print(
        f"   Average Characters per Episode: {content['average_characters_per_episode']:,}"
    )
